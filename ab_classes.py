...
class SendingContact(SendingToUsers):
    ...
    def sending(self,language):
        if language:
            return f"Added contact '{self.contact}' with phone: {self.phone if self.phone else '-'}, email: {self.email} and address: {self.address}"
        return f"Додано контакт '{self.contact}' з телефоном: {self.phone if self.phone else '-'}, електронною поштою: {self.email} та адресою: {self.address}"
    
    def sendADd(self,language):
        if language:
            return f"Added phone number: {self.phone}, email: {self.email} and address: {self.address} for existing contact '{self.contact}'"
        return f"Для існуючого контакту '{self.contact}' додано номер телефону: {self.phone}, електронну пошту: {self.email} та адресу: {self.address}"
    
    def sendDel(self,language):
        if language:
            return f"Contact {self.contact} Removed!"
...
class AddressBook(UserDict):
    def load_from_file(self, filename) -> None:
        try:
            with open("phonebook.JSON", "r") as db:
                data = json.load(db)
        except EOFError:
            pass
        for rec in data:
