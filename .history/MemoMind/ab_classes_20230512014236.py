...
class AddressBook(UserDict):
    def load_from_file(self, filename):
        try:
            ...
            pass
        for rec in data:
            record  = Record(Name(data[rec]['name']), [Phone(phone) for  phone in data[rec]['phones']], Email(data[rec]['email']) if (data[rec]['email']) else None, Address(data[rec]['address']), Birthday(data[rec]['birthday']))
