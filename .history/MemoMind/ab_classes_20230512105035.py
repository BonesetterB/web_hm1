...
class AddressBook(UserDict):
    ...
    def save_to_file(self, filename):
        ...
        for rec in self.data.values():
            ...
            address = rec.address.value if rec.adress is not None else None
            birthday = str(rec.birthday.value) if rec.birthday is not None else None     
            data_value = {rec.name.value:{"name":rec.name.value, "phones":[phones], "email":email, "address":address, "birthday":birthday}}
