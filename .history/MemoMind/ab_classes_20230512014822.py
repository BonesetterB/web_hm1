...
class AddressBook(UserDict):
    def load_from_file(self, filename):
        ...
        for rec in data:
            ...
            phones = [Phone(phone) for  phone in data[rec]['phones']]
            email = Email(data[rec]['email']) if data[rec]['email'] else None
            address = Address(data[rec]['address']) if data[rec]['address'] else None
            birthday = Birthday(data[rec]['birthday']) if data[rec]['birthday'] else None
            record  = Record(name, phones, email, address, birthday)
    ...
    def save_to_file(self, filename):
        ...
        for rec in self.data.values():
            data_value = {rec.name.value:{"name":rec.name.value, "phones":rec.phones, "email":rec.email, "address":rec.address, "birthday":rec.birthday}}
